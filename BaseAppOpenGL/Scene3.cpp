...
int CScene3::DrawGLScene(void)	// Função que desenha a cena
{
	...
	auxSolidBox(1.0, 10.0, 1.0);
	glPopMatrix();  //Reseta o Plano carteziano ao centro


	glColor3ub(0, 255, 0);

	glPushMatrix();
	glTranslatef(fMoveX, fMoveY, fMoveZ);  //Translação sempre vem antes da rotação
	glRotatef(270.0, 1.0f, 0.0f, 0.0f);
	auxSolidCone(2.0, 5.0);
	glPopMatrix();
	...
	glPushMatrix();
	glTranslatef(10.0f, 0.0f, 0.0f);
	glScalef(fMoveScale, fMoveScale, fMoveScale);
	auxSolidTeapot(2.0);
	glPopMatrix();
...
void CScene3::JumpWithSpace()
{
	int temp = 1000;
	for (float i = 0.0; i <= 5.0; i = i + 0.005) {
		while (temp) { temp--; };
		fMoveY += 0.005;
	}
	//for (float i = 5.0; i >= 0.0; i = i - 0.005) {
	//	fMoveY -= 0.005;
	//}
	fMoveY = 10.0f;
}
